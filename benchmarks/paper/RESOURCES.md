...
Seven such points give about 9 h. Together with the complete runs,
...
The stored interacting references stop after 4–7 particle-number shells.
The largest required momentum block is at U1=0.25 (input U1=0.25, U2=0):
...
The supplied convergence policies and particle cutoffs are used for the
...
Use the same `--scope`, `--model`, `--python-ed`, and `--threads` as the initial command.
Completed stages are checked and skipped. An interrupted chain is moved aside
in scratch and restarted from its supplied input; its partial samples are never
