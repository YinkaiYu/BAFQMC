...
\[
H_\mu = H_t + H_U + H_\Delta - \mu (N_b+N_c).
\]

With \(b_i\) and \(c_i\) the two boson flavors,

\[
H_t = t\sum_{\langle ij\rangle}
\left(b_i^\dagger b_j+b_j^\dagger b_i+c_i^\dagger c_j+c_j^\dagger c_i\right),
\]
...
\[
H_\Delta = \Delta\sum_i
\left(b_i^\dagger c_i^\dagger+b_i c_i\right).
\]

The code reads \(U_1\), \(U_2\), \(\mu\), and \(\Delta\) from `paramC_sets.txt`; \(t\) is `RT=1` in `src/calc_basic.f90`. In the ED scripts, the interaction is equivalently

\[
(U_1+U_2)(n_{b,i}^2+n_{c,i}^2)
+2(U_1-U_2)n_{b,i}n_{c,i}.
\]

### Manuscript notation and pair phase

The main-text benchmark sets \(U_1=0\) and denotes \(U_2\) by \(U\).
The paired scan has \(U=1\), \(\mu=-5\), \(\beta=4\), and
\(\Delta=0,0.05,\ldots,0.30\). Its density, physical energy, and structure
factors form panels (e–h) of the combined benchmark figure.

The manuscript writes the real pair term as
\(-\Delta\sum_i(b_i^+c_i^++b_ic_i)\). The implementation above uses the
phase convention \(b_{\rm code}=b_{\rm paper}\),
\(c_{\rm code}=-c_{\rm paper}\), at the same positive value of \(\Delta\).
Hopping and density interactions are unchanged by this transformation.
The four plotted observables \(\rho\), \(-E\), \(S_{\rm SF}(K)\), and
\(S_{\rm DW}(K)\) therefore use the stored values directly. The anomalous
amplitude per site is
\(P_{\rm paper}=N_s^{-1}\sum_i\langle b_i c_i+b_i^+c_i^+\rangle_{\rm paper}
=-\texttt{pair_equal}\). Its physical energy contribution per site is
\(e_\Delta=-\Delta P_{\rm paper}=\Delta\,\texttt{pair_equal}\),
and the total contribution is \(E_\Delta=N_s e_\Delta\).
Both the BAFQMC and ED inputs retain the implementation's real coefficient.

For this \(U_1=0\) model, the condition \(\mu<-3t-|\Delta|\) guarantees
a finite trace for every real auxiliary-field configuration, as proved in the
SM subsection "Convergence throughout the auxiliary-field domain". All seven
...
e^{\alpha_2\phi_2(n_b-n_c)},
\qquad
\alpha_2=i\sqrt{2U_2\Delta\tau}.
\]
...
At a zero coupling the corresponding HS coefficient vanishes.
...
\[
H=\frac12\tilde\chi^\dagger \tilde A\tilde\chi,\qquad
\tilde\chi=(b,c^\dagger)^T.
\]
...
\lambda_2=i\bar\alpha_2\phi_2,\qquad
\bar\alpha_2=\sqrt{2U_2\Delta\tau}.
\]

In the Fortran code the second coefficient is stored equivalently as
\(\lambda_2=\alpha_2\phi_2\) with
\(\alpha_2=i\sqrt{2U_2\Delta\tau}\). The HS density term satisfies

\[
\lambda_1(b^\dagger b+c^\dagger c)
+\lambda_2(b^\dagger b-c^\dagger c)
=
-\frac12\chi^T\Omega
\begin{pmatrix}
...
\((b,c,b^\dagger,c^\dagger)\), its blocks represent

\[
\tilde G=
\left\langle
\begin{pmatrix}
bb^\dagger & bc^\dagger & -bb & -bc\\
cb^\dagger & cc^\dagger & -cb & -cc\\
b^\dagger b^\dagger & b^\dagger c^\dagger & -b^\dagger b & -b^\dagger c\\
c^\dagger b^\dagger & c^\dagger c^\dagger & -c^\dagger b & -c^\dagger c
\end{pmatrix}
\right\rangle .
\]
...
\(G_{b^\dagger,b}=-\texttt{Prop\%Gr(bdag,bdag)}\).

Define

\[
n_b(i,j)=G_{b^\dagger,b}(i,j),\qquad
n_c(i,j)=G_{c^\dagger,c}(i,j).
\]
...
D_{bb}(i,j)=
n_b(i,i)n_b(j,j)
+G_{b^\dagger,b}(i,j)G_{b,b^\dagger}(i,j)
+G_{b^\dagger,b^\dagger}(i,j)G_{b,b}(i,j),
\]

\[
D_{cc}(i,j)=
n_c(i,i)n_c(j,j)
+G_{c^\dagger,c}(i,j)G_{c,c^\dagger}(i,j)
+G_{c^\dagger,c^\dagger}(i,j)G_{c,c}(i,j),
\]

\[
D_{bc}(i,j)=
n_b(i,i)n_c(j,j)
+G_{b^\dagger,c^\dagger}(i,j)G_{b,c}(i,j)
+G_{b^\dagger,c}(i,j)G_{b,c^\dagger}(i,j).
\]
...
\left[G_{b,c}(i,i)+G_{b^\dagger,c^\dagger}(i,i)\right].
\]

`squareOcc` intentionally preserves the no-pairing density-product diagnostic. `local_numsquare` is the physical local \(\langle n_b^2+n_c^2\rangle/N_s\) observable compared to ED.
...
| `density_site_total` | \(\rho_i=\mathrm{Re}[G_{b^\dagger b}(i,i)+G_{c^\dagger c}(i,i)]\) | site vector | IPR postprocessing | fixed-width row | no | yes |
...
| `squareOcc` | \(N_s^{-1}\sum_i \mathrm{Re}[n_b(i,i)^2+n_c(i,i)^2]\) | per site | density-product diagnostic only | `mean` | yes | no |
...
| `pair_equal` | \(N_s^{-1}\sum_i\mathrm{Re}\langle b_i c_i+b_i^\dagger c_i^\dagger\rangle\) | per site | \(N_s^{-1}\partial F/\partial\Delta\) | `mean` | zero check | yes |
...
| `sf_K` | \(N_s^{-2}\sum_{ij}e^{iK\cdot(r_i-r_j)}\langle b_i^\dagger b_j+c_i^\dagger c_j\rangle\) | complex scalar | `S_SF_K` | real/imag block stats | no | yes |
...
| `psf_Gamma` | \(N_s^{-2}\sum_{ij}\langle b_i^\dagger c_i^\dagger c_j b_j\rangle\) | complex scalar | `S_PSF_Gamma` | real/imag block stats | no | yes |
...
channels:
\[
S_{\rm SF}(K)=\frac{1}{N_s}\sum_{\alpha=b,c}\langle a^\dagger_{K,\alpha}a_{K,\alpha}\rangle,
\qquad
