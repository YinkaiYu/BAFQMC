...
两个通道可以独立设置：取 $`U_2=0`$ 得到只含相对密度通道的情形；固定 $`U_1=1`$ 并改变 $`U_2`$
即可研究总密度通道。输入的配对强度就是论文中的实数
...
和[任务指南](./agent-workflows.md)提供代码地图、物理约定、运行命令和验证方法：
...
```text
获取 BAFQMC，阅读 AGENTS.md，使用仓库默认命令完整复现论文 benchmark，
将大型原始链放在 Git 之外，保留处理后的 benchmark 数据，并报告资源预算、输出路径、
最终比较表和图。
```

```text
如有需要先配置 https://github.com/YinkaiYu/BAFQMC，然后使用论文参数和种子完整复现
所有 benchmark，将新结果保存在独立目录中，完成后给我看两张图。
```
