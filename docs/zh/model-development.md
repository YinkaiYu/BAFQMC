...
用算符形式写出相互作用，包括其线性项和常数项。例如，$`n^2=n(n-1)+n`$，因此用一个替换另一个时也会改变化学势的贡献。在整个实现过程中，分别定义物理能量和巨正则能量。对于论文当前约定，用 $`b,c`$ 表示两个组分，并写成
$`H_{\mathrm{int}}=U_1(n_b-n_c)^2+U_2(n_b+n_c)^2`$，其中 $`U_1\geq0`$、$`U_2\leq0`$。主 benchmark 取 $`U_1=U`$、$`U_2=0`$；配对 benchmark 取 $`U_1=1`$、$`U_2=0`$；补充材料固定 $`U_1=1`$ 并扫描 $`U_2<0`$。
...
最简单的现有构造给出了一个明确的例子。对于两个组分的等实跃迁，$`U_1\geq0`$ 和 $`U_2\leq0`$ 在**相同**场值下生成共轭的单组分因子。若 $`B_c(\phi)=B_b(\phi)^*`$，则在收敛热迹域内，
...
仅凭实数 Bogoliubov 频率并不能建立该能量矩阵的正定性。对于当前具有均匀在位配对和实数跃迁的 $`U_2=0`$ 构造，相应的充分界为
...
配对表示还有一个标量正规序贡献。对于当前的总密度 HS 场 $`x_2=\sqrt{-2U_2\Delta\tau}\,\phi_2`$，标量因子为 $`e^{-x_2}`$，一次提议对 `ratio_constant` 贡献 $`e^{-(x_2'-x_2)}`$。对不同的二次生成元或 Nambu 约定，需重新推导此因子。
...
- **通道符号编码算符。** 配对版 `opU_set` 对非正的 $`U_2`$ 通道选择总密度，对非负的 $`U_1`$ 通道选择相对密度。相反符号或附加算符需要显式的通道定义；重命名 `Op_U1` 或更改 JSON 数字并不会改变该调度。
...
对于等实数跃迁，该变化保留共轭组分关系和原始在位 HS 通道。对于 $`U_2=0`$ 和均匀实数在位配对，一个简单的充分热力学界为

```math
\mu<-2\left(|t_1|+|t_2|+|t_3|\right)-|\Delta|.
```

它由跃迁谱的下界推出，不必等于准确的能带最小值。作为具体的实现测试，使用 $`(t_1,t_2,t_3)=(1,0.8,0.35)`$、$`\mu=-5`$、$`\beta=1`$，首先令 $`U_1=U_2=0`$。测试 $`\Delta=0`$，以及对配对扩展测试 $`\Delta=0.1`$。然后在 $`U_1=0.7`$ 处检验现有的相对密度相互作用。这些是提议的扩展测试，不是捆绑的可运行情形。
