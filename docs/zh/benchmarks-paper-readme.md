...
```text
data/index.json                  所选算例、参数、绘图值和元数据
...
data/<model>/ed/<case>.json      紧凑型 ED 参考结果
data/<model>/raw/<case>.tar.gz   可选的本地原始链（Git 忽略）
analysis.py                      原始数据统计、参考值、表格、绘图调度
plot_manuscript.py               主 U1/Delta 网格和补充 U2 扫描
plot_number.py                   2×4 粒子数守恒布局
plot_pairing.py                  1×4 配对布局
plot_style.py                    共享图像样式
```
...
对新测量结果进行分块，并从新结果生成两张图。已跟踪的处理后数据保留在 `data/` 中。
...
创建新模型方案时不要覆盖已跟踪的输入文件；请将其复制到自己的配置目录中。
复现所需的源文件、计算方案输入、处理后数据和参考实现均包含在本仓库中。
