...
contains
    subroutine AuxConf_make(this)
        ...
        allocate(this%phi_list(Naux, Nsite, Ltrot))
        return
...
! Local: 
        real(kind=8), dimension(Naux, NsiteTherm, LtrotTherm) :: phi_list_therm
...
!#define DEC
        include 'mpif.h'
! Arguments: 
        real(kind=8), dimension(Naux, NsiteTherm, LtrotTherm), intent(inout) :: phi_list_therm
        integer, intent(out) :: iseed
! Local: 
        ...
        real(kind=8), dimension(Naux, NsiteTherm, LtrotTherm) :: phi_list_itmp
...
!   Setup node I and send data.
					...
					call MPI_SEND(phi_list_itmp, NsiteTherm*Naux*LtrotTherm, MPI_Real8, N, N+1024, MPI_COMM_WORLD, IERR)
...
! Setup Node 0
                ...
                do N = 1, ISIZE - 1
                    ...
                    enddo
                    call MPI_SEND(itmp, 1, MPI_Integer, N, N, MPI_COMM_WORLD, IERR)
					call MPI_SEND(phi_list_itmp, Naux*NsiteTherm*LtrotTherm, MPI_Real8, N, N+1024, MPI_COMM_WORLD, IERR)
                enddo
            endif
        else
            call MPI_RECV(iseed, 1, MPI_Integer, 0, IRANK, MPI_COMM_WORLD, STATUS, IERR)
			call MPI_RECV(phi_list_therm, Naux*NsiteTherm*LtrotTherm, MPI_Real8, 0, IRANK + 1024, MPI_COMM_WORLD, STATUS, IERR)
    ...
    subroutine conf_set(phi_list, itmp)
        real(kind=8), dimension(Naux, NsiteTherm, LtrotTherm), intent(out) :: phi_list
...
! Local: 
    ...
    subroutine conf_transfer(phi_list, phi_list_therm, Latt)
        real(kind=8), dimension(Naux, Nsite, Ltrot), intent(out) :: phi_list
        real(kind=8), dimension(Naux, NsiteTherm, LtrotTherm), intent(in) :: phi_list_therm
        class(pairingTriangularLattice), intent(in) :: Latt
        integer :: nt, ntt, nx, ny, ii, iit, nc, no, n
        integer :: cell_list_therm(LqTherm, 1:2), inv_cell_list_therm(NlxTherm, NlyTherm)
        integer :: site_list_therm(NsiteTherm, 1:2), inv_site_list_therm(LqTherm, Norb)
! set therm list
        nc = 0
        do no = 1, Norb
            do n = 1, LqTherm
                nc = nc + 1
                site_list_therm(nc, 1) = n
                site_list_therm(nc, 2) = no
                inv_site_list_therm(n, no) = nc
            enddo
        enddo
        nc = 0
        do ny = 1, NlyTherm
...
! copy Conf
        do no = 1, Norb
            do nt = 1, LtrotTherm
                do ny = 1, NlyTherm
                    do nx = 1, NlxTherm
                        iit = inv_site_list_therm(inv_cell_list_therm(nx, ny), no)
                        ii  = Latt%inv_site_list(  Latt%inv_cell_list(nx, ny), no)
            ...
            do nt = 1, LtrotTherm
                do ny = 1, NlyTherm
                    do nx = NlxTherm+1, Nlx
                        iit = inv_site_list_therm(inv_cell_list_therm(nx - NlxTherm, ny), no)
                        ii  = Latt%inv_site_list(  Latt%inv_cell_list(nx, ny), no)
                        phi_list(:, ii, nt) = phi_list_therm(:, iit, nt)
                    enddo
                enddo
        ...
        do no = 1, Norb
            do nt = 1, LtrotTherm
                do ny = NlyTherm+1, Nly
                    do nx = 1, NlxTherm
                        iit = inv_site_list_therm(inv_cell_list_therm(nx, ny - NlyTherm), no)
                        ii  = Latt%inv_site_list(  Latt%inv_cell_list(nx, ny), no)
                        phi_list(:, ii, nt) = phi_list_therm(:, iit, nt)
                    enddo
                enddo
        ...
        do no = 1, Norb
            do nt = 1, LtrotTherm
                do ny = NlyTherm+1, Nly
                    do nx = NlxTherm+1, Nlx
                        iit = inv_site_list_therm(inv_cell_list_therm(nx - NlxTherm, ny - NlyTherm), no)
                        ii  = Latt%inv_site_list(  Latt%inv_cell_list(nx, ny), no)
                        phi_list(:, ii, nt) = phi_list_therm(:, iit, nt)
                    enddo
                enddo
...
! Local:
        ...
        if (IRANK .NE. 0) then
			...
			call MPI_SEND(Conf%phi_list, Nsite*Naux*Ltrot, MPI_Real8, 0, IRANK+1024, MPI_COMM_WORLD, IERR)
        ...
        if (IRANK == 0) then
            ...
            do N = 1, ISIZE - 1
				...
				call MPI_RECV(Conf%phi_list, Nsite*Naux*Ltrot, MPI_Real8, N, N+1024, MPI_COMM_WORLD, STATUS, IERR)
                write(35,*) iseed
                do nt = 1, Ltrot
                    do ii = 1, Nsite
                        do ns = 1, Naux
