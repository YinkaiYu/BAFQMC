...
contains
    ...
    subroutine Local_sweep(this, Prop, WrList, iseed, is_beta, toggle)
        ...
        if (abs(RU1) > Zero) call Op_U1%Phase_U_local%ratio()
        if (abs(RU2) > Zero) call Op_U2%Phase_U_local%ratio()
        return
    ...
    subroutine Local_control_print(toggle)
        include 'mpif.h'
        logical, intent(in) :: toggle
        real(kind=8) :: collect, collect_count
        if (abs(RU1) > Zero) then
            ...
            collect = 0.d0
            call MPI_Reduce(Op_U1%Phase_U_local%sum_abs_arg_proposed, collect, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            collect_count = 0.d0
            call MPI_Reduce(Op_U1%Phase_U_local%count_proposed, collect_count, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            if (IRANK == 0 .and. collect_count > 0.d0) write(50,*) 'Phase_U1_mean_abs_arg_proposed                :', collect / collect_count
            collect = 0.d0
            call MPI_Reduce(Op_U1%Phase_U_local%sum_abs_arg_accepted, collect, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            collect_count = 0.d0
            call MPI_Reduce(Op_U1%Phase_U_local%count_accepted, collect_count, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            if (IRANK == 0 .and. collect_count > 0.d0) write(50,*) 'Phase_U1_mean_abs_arg_accepted                :', collect / collect_count
            collect = 0.d0
            call MPI_Reduce(Op_U1%Phase_U_local%branch_pi_count, collect, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            collect_count = 0.d0
            call MPI_Reduce(Op_U1%Phase_U_local%count_proposed, collect_count, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            if (IRANK == 0 .and. collect_count > 0.d0) write(50,*) 'Phase_U1_branch_pi_fraction                   :', collect / collect_count
            if (IRANK == 0) write(50,*) 'Phase_U1_local_proposals                      :', collect_count
        endif
        if (abs(RU2) > Zero) then
            ...
            collect = 0.d0
            call MPI_Reduce(Op_U2%Phase_U_local%sum_abs_arg_proposed, collect, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            collect_count = 0.d0
            call MPI_Reduce(Op_U2%Phase_U_local%count_proposed, collect_count, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            if (IRANK == 0 .and. collect_count > 0.d0) write(50,*) 'Phase_U2_mean_abs_arg_proposed                :', collect / collect_count
            collect = 0.d0
            call MPI_Reduce(Op_U2%Phase_U_local%sum_abs_arg_accepted, collect, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            collect_count = 0.d0
            call MPI_Reduce(Op_U2%Phase_U_local%count_accepted, collect_count, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            if (IRANK == 0 .and. collect_count > 0.d0) write(50,*) 'Phase_U2_mean_abs_arg_accepted                :', collect / collect_count
            collect = 0.d0
            call MPI_Reduce(Op_U2%Phase_U_local%branch_pi_count, collect, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            collect_count = 0.d0
            call MPI_Reduce(Op_U2%Phase_U_local%count_proposed, collect_count, 1, MPI_Real8, MPI_SUM, 0, MPI_COMM_WORLD, IERR)
            if (IRANK == 0 .and. collect_count > 0.d0) write(50,*) 'Phase_U2_branch_pi_fraction                   :', collect / collect_count
            if (IRANK == 0) write(50,*) 'Phase_U2_local_proposals                      :', collect_count
        endif
